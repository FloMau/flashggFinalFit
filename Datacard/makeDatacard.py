...
if opt.prune:
  ...
  if opt.doTrueYield:
    ...
    mask = (pd.to_numeric(data['nominal_yield'], errors='coerce')<opt.pruneThreshold*data.apply(lambda x: catYields[x['cat']], axis=1))&(data['type']=='sig')&(~data['cat'].str.contains('NOTAG'))
    data.loc[mask,'prune'] = 1
