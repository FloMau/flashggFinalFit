...
BASE_WS=/net/data_cms3a-1/mausolf/HttCPAnalysis/finalFitPreparation/outputForFinalFits/workspaces
...
for era in "${eras[@]}"; do
  ...
  # 3) package up all the toy/workspace outputs
  python3 RunPackager.py \
    --cats auto \
    --inputWSDir ${BASE_WS}/${era}/ws_signal/ \
    --year ${era} \
    --batch local \
    --massPoints 125 \
    --exts tth_th_analysis_${era} \
    --mergeYears
...
for era in 2022preEE 2022postEE 2023preBPix 2023postBPix; do
  for f in /net/data_cms3a-1/mausolf/HttCPAnalysis/finalFitPreparation/outputForFinalFits/workspaces/${era}/ws_signal/*.root; do
